// --- TÜM GLOBAL DURUM VE DEĞİŞKENLER (EN BAŞTA TANIMLANMALI) ---
let currentStage = 1;
...
// --- OYUN MODLARI DURUM DEĞİŞKENLERİ ---
// Kart Eşleştirme
let flippedCards = [];
let matchedPairs = 0;
...
// Kelime Avcısı (Bouncing Balonlar)
let bouncingObjects = [];
let rafId = null;
...
const LANGUAGE_CONFIG = {
    arabic: {
        words: () => (typeof arabicWords !== 'undefined' ? arabicWords : []),
        voice: 'ar'
    },
    english: {
        words: () => (typeof englishWords !== 'undefined' ? englishWords : []),
        voice: 'en'
    }
};
...
function selectLanguage(lang, event) {
    ...
    // Güvenli element tespiti (event, currentTarget veya DOM sorgusu)
    let wrapper = null;
    try {
        if (event) {
...
function doSelectLanguage(lang) {
    try {
        ...
        if (LANGUAGE_CONFIG[selectedLanguage] && typeof LANGUAGE_CONFIG[selectedLanguage].words === 'function') {
            activeWords = LANGUAGE_CONFIG[selectedLanguage].words() || [];
        } else if (typeof arabicWords !== 'undefined') {
            activeWords = arabicWords;
        }
        ...
        if (titleEl) titleEl.textContent = `🌸 ELİF İNCİ'NİN DİLLER BAHÇESİ 🌺`;
        if (subtitleEl) subtitleEl.textContent = `${langNames[selectedLanguage] || 'Arapça'} Öğrenmeye Hazır mısın?`;

        showScreen('main-menu');
    } catch (err) {
        console.error('doSelectLanguage hatası:', err);
        // Hata olsa bile ana menüyü kesinlikle aç!
        showScreen('main-menu');
    }
}
...
function loadVoices() {
    try {
        ...
        if (voices.length > 0) {
            voicesLoaded = true;
            arabicVoice = voices.find(v => v && v.lang && (v.lang.startsWith('ar') || v.lang.toLowerCase().includes('ar-'))) ||
                voices.find(v => v && v.name && v.name.toLowerCase().includes('arabic'));
        ...
        console.warn('loadVoices hatası:', e);
    }
}
...
function showScreen(id) {
    try {
        ...
        if (targetScreen) {
            ...
            console.warn('Hedef ekran bulunamadı:', id);
        }
        ...
        try {
            if (id !== 'game-colors' && typeof stopBalloonGame === 'function') stopBalloonGame();
            if (id !== 'game-train' && typeof stopTrainGame === 'function') stopTrainGame();
        } catch (e) { }

        // Ana menü veya dil seçiminde geri tuşu ve yıldız sayacı gizle
        if (homeBtn && starCounter) {
            if (id === 'main-menu' || id === 'language-screen' || id === 'game-complete') {
        ...
        if (id === 'stage-select' && typeof initStageGrid === 'function') {
            initStageGrid();
        }
        ...
        console.error('showScreen hatası:', err);
    }
}
